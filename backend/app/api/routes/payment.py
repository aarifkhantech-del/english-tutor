...
from app.models.db_models import Payment, Subscription
...
async def create_order(payload: CreateOrderIn):
    # Step 1: Validate amount >= 100 paise (₹1.00)
...
async def verify_payment(payload: VerifyPaymentIn, db: Session = Depends(get_db)):
    ...
    if not hmac.compare_digest(expected_signature, signature):
        logger.warning(
            "Razorpay signature verification failed for order %s and payment %s",
            order_id,
            payment_id,
        )
        raise HTTPException(
    ...
    # Optional DB update if order exists in database
    try:
        db_payment = db.query(Payment).filter(Payment.gateway_order_id == order_id).first()
        if db_payment:
            db_payment.status = "captured"
            db_payment.gateway_payment_id = payment_id
            db_payment.gateway_signature = signature
            if db_payment.subscription_id:
                sub = db.query(Subscription).filter(Subscription.id == db_payment.subscription_id).first()
                if sub:
        ...
        logger.warning("DB record update after verification encountered warning: %s", db_err)
